...
<body><main>
  ...
  <section data-step="0" class="active"><h2>欢迎</h2><p>向导会自动检查数据库、配置 Authentik，并可连接 OpenAI-compatible 或 Ollama。数据库地址和配置加密密钥必须预先写入服务器配置。</p><div class="notice">当前是未安装状态，任何能访问此页面的人都可以发起首次安装。请尽快完成安装，或在反向代理中临时限制来源 IP。安装完成后向导将永久锁定。</div><div class="actions"><span></span><button id="diagnoseButton" type="button">重新检查环境</button></div><div id="diag" class="result">正在加载页面脚本并检查环境……</div></section>
