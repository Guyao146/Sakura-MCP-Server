...
<body><main>
  ...
  <section data-step="1"><h2>Authentik 身份认证</h2><div class="notice">先填写 Authentik 根地址和应用 Slug，向导会从标准 OpenID Configuration 自动回填端点。Audience 和 Client ID 仍需按 Provider 配置填写。</div><div class="grid"><div><label>Authentik 地址</label><input id="authBaseUrl" type="url" placeholder="https://login.example.com"></div><div><label>应用名称（Application Slug）</label><input id="authApplicationSlug" placeholder="sakura-mcp"></div></div><div class="actions"><span></span><button id="discoverAuthentikButton" class="secondary" type="button">获取 OpenID 配置</button></div><div id="discoveryResult" class="result">填写地址和应用名称后自动获取。</div><div class="grid"><div><label>Issuer</label><input id="issuer" placeholder="自动回填"></div><div><label>Audience</label><input id="audience" placeholder="https://mcp.example.com"></div></div><label>Client ID（Public + PKCE）</label><input id="clientId" placeholder="Authentik OAuth Client ID"><label>JWKS URI</label><input id="jwksUri" placeholder="自动回填"><div class="grid"><div><label>Authorization URL</label><input id="authorizationUrl" placeholder="自动回填"></div><div><label>Token URL</label><input id="tokenUrl" placeholder="自动回填"></div></div><label>UserInfo URL（可选）</label><input id="userinfoUrl" placeholder="自动回填"><label>Scope Claim</label><input id="scopeClaim" value="scope"><label>首位系统管理员邮箱</label><input id="adminEmail" type="email" placeholder="admin@example.com"><div class="actions"><button id="authBackButton" class="secondary" type="button">上一步</button><button id="authTestButton" type="button">测试并继续</button></div><div id="authResult" class="result">尚未测试。</div></section>
