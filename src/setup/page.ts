...
function providerData(){const provider=$('provider').value;const embeddingBase=$('embeddingBase').value.trim();const embedding=embeddingBase?{embedding:{baseUrl:embeddingBase,apiKey:$('embeddingKey').value||undefined,model:$('embeddingOnlyModel').value||undefined}}:{};if(provider==='none')return embedding;const data={baseUrl:$('providerBase').value,chatModel:$('chatModel').value||undefined,embeddingModel:$('embeddingModel').value||undefined};return provider==='openai'?{openaiCompatible:{...data,apiKey:$('providerKey').value||undefined},...embedding}:{ollama:data,...embedding}}
...
async function testProviderAndContinue(){const embeddingBase=$('embeddingBase').value.trim();if($('provider').value==='none'&&!embeddingBase){go(3);return}const button=$('providerContinueButton');button.disabled=true;try{const data=await api('test-provider',providerData());$('providerResult').className='result ok';$('providerResult').textContent='✓ '+data.provider+' 连接成功';go(3)}catch(error){$('providerResult').className='result bad';$('providerResult').textContent='✗ '+message(error)}finally{button.disabled=false}}
