...
describe('application version and update checks', () => {
  ...
  it('checks the latest GitHub release and caches the result', async () => {
    ...
    const checker = new UpdateChecker('0.2.17', 60_000, fetcher, () => 1_000);
    await expect(checker.check()).resolves.toMatchObject({
      currentVersion: '0.2.17', latestVersion: '0.3.0', updateAvailable: true,
  ...
  it('rejects invalid or unsuccessful release responses', async () => {
    const failed = new UpdateChecker('0.2.17', 60_000, vi.fn().mockResolvedValue(new Response('', { status: 503 })));
    await expect(failed.check()).rejects.toThrow('503');
    const invalid = new UpdateChecker('0.2.17', 60_000, vi.fn().mockResolvedValue(new Response('{}', { status: 200 })));
